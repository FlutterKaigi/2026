...
参加者は **アプリのログイン必須**(Google / Apple / メール)。匿名認証では参加できず、
ルールでも `firebase.sign_in_provider != 'anonymous'` で弾く。参加・回答の記録は
ログイン中のアカウントの `uid` に紐づく。

導線はアカウントタブに置き、サインイン後のアカウント画面にクイズ大会のタイルを出す
(未ログイン時はタイル自体が出ない)。ニックネームはアカウントの表示名を初期値として
編集可能にし、入力の手間を減らす。
...
クイズ配下は全パスでログイン必須(`signedInWithAccount()` = サインイン済み かつ 匿名でない)。

| パス | read | write(クライアント) |
|---|---|---|
| `quizEvents`, `teams` | ログインユーザー | 不可(管理者のみ) |
| `questions` | ログインユーザー、get のみ(draft は管理者のみ)。list は管理者のみ | 不可(管理者のみ) |
| `questions/*/secret` | **不可**(管理者のみ) | 不可 |
| `participants/{uid}` | ログインユーザー(ニックネームのみ) | 本人のみ create。イベント `status == 'registration'` 中のみ。`displayName` は 1〜20 文字 |
| `participantAccounts/{uid}` | **本人と管理者のみ** | 本人のみ create。値は ID トークンと一致必須 |
| `answers/{qid}_{tid}` | **自チームメンバーのみ**(get のみ。list は管理者のみ) | 自チームメンバーのみ update(create / delete は管理者のみ) |

### アカウント紐づけ

参加記録は `participants/{uid}`(ドキュメント ID = uid)で既にアカウントに紐づくが、
メールアドレスなど本人にしか見せない情報は `participantAccounts/{uid}` に分けて保存する
(`participants` はログインユーザーなら誰でも読めるため)。`email` / `signInProvider` は
ルールが ID トークンと突き合わせるので、なりすましや偽装した申告はできない。
`participants` の create は `participantAccounts` の存在を要求し、紐づけのない参加記録が
生まれないようにしている。
...
ログイン必須にしたことで使い捨ての匿名 uid による多重参加はできなくなった。
複数アカウントの作成自体は技術的に防げないが、回答が締切まで自チーム以外に見えないため
...
`feature/quiz/` として追加。ルートはアカウントタブ配下の `/account/quiz` と
`/account/quiz/:eventId`(TypedGoRoute)。導線はサインイン後のアカウント画面のタイル 1 本。
未ログインで開かれた場合はサインイン案内を出してアカウント画面へ戻す。

画面は `quiz_events.status` と自分の参加状態で切り替わるステートマシン:

1. **参加登録**: ニックネーム入力(アカウントの表示名を初期値) → 参加ボタン
   (現在の参加人数をリアルタイム表示)。
...
3. `apps/app`: アカウントタブの導線 + `/account/quiz` ルート + 参加登録〜チーム発表
