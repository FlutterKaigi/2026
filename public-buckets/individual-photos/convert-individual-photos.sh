...
# =============================================================================
# FlutterKaigi 2026 個人スポンサー写真 変換スクリプト
# =============================================================================
# source/ 配下の svg / png / jpg / jpeg を webp に変換し、
# ファイル名を UUID（ランダムで一意なキー）にして photos/ に出力する。
#
# public-buckets/sponsor-logos/ (企業スポンサーロゴ用) と同じ仕組みだが、
# R2 上のプレフィックスを分けて個人スポンサーを独立して管理するための
# 姉妹ツール。
#
# 出力された photos/ 配下の webp は upload-individual-photos.sh で
# R2 公開バケットにアップロードできる。
#
# 変換元 → 出力 → 公開URL の対応は photos-manifest.tsv に追記される。
# （UUID ファイル名だけではどの個人スポンサーの写真か分からないため）
#
# 必要なツール:
...
#
# オプション（環境変数）:
#   WEBP_LOSSLESS=1   ロスレス変換（ロゴの輪郭・透過を保持）
#   WEBP_LOSSLESS=0   ロッシー変換（既定。顔写真は透過が不要で、ロスレスに
#                     すると企業ロゴ用と違いファイルサイズが増えるだけなので
#                     既定をロッシーにしている）
#   WEBP_QUALITY=90   ロッシー時の品質 (0-100)（既定）
#   MAX_WIDTH=320     出力の最大幅(px)。svg はこの幅でラスタライズ、
#                     png/jpg はこの幅を超える場合のみ縮小（拡大はしない）
#                     ホームの円形アバター表示(96px、Retinaで最大288px相当)と
#                     OGPカードへの合成(最大268px相当)の両方を余裕を持って
#                     カバーできるサイズ。企業ロゴ用(既定1024px)より小さいのは、
#                     個人スポンサーの写真がこの2箇所以外に使われないため。
#   CLEAN=1           全件リビルド。photos/ の *.webp と manifest を削除し、
#                     全 source を新しい UUID で変換し直す（増分スキップを無効化）
# =============================================================================

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_DIR="$SCRIPT_DIR/source"
DEST_DIR="$SCRIPT_DIR/photos"
MANIFEST="$SCRIPT_DIR/photos-manifest.tsv"
PUBLIC_BASE="https://2026-bucket.flutterkaigi.jp/individual"

WEBP_LOSSLESS="${WEBP_LOSSLESS:-0}"
...
if [ "$CLEAN" = "1" ]; then
  echo "CLEAN=1: photos/ の *.webp と manifest を削除し、全件リビルドします"
...
# git や manifest 側の NFC(合成形) と完全一致しなくなる。
...
while IFS= read -r -d '' src; do
  ...
  if [ "$ext" = "svg" ]; then
    ...
    tmp_png="$(mktemp -t individualphoto).png"
...
echo ""
echo "次のステップ: アップロード"
echo "  ./public-buckets/individual-photos/upload-individual-photos.sh"
