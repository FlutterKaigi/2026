...
void main() {
  ...
  setUp(() => tmp = Directory.systemTemp.createTempSync('captions_config_test'));
  ...
  test('values load from the CAPTIONS_CONFIG file, with number/boolean coercion', () {
    final path = writeConfig('{"GEMINI_API_KEY": "file-key", "INGEST_TOKEN": "file-token", "PORT": 8085}');
  ...
  test('env wins over the config file', () {
    ...
    final config = Config.load({'CAPTIONS_CONFIG': path, 'INGEST_TOKEN': 'env-token'});
    expect(config.ingestToken, 'env-token', reason: 'env must override the file');
    expect(config.port, 8085, reason: 'keys absent from env still come from the file');
  });

  test('an explicit CAPTIONS_CONFIG pointing to a missing file fails fast', () {
    expect(() => Config.load({'CAPTIONS_CONFIG': '${tmp.path}/nope.json'}), throwsA(isA<ConfigException>()));
  });

  test('invalid JSON, non-object JSON, and nested values fail fast', () {
    expect(() => Config.load({'CAPTIONS_CONFIG': writeConfig('not json')}), throwsA(isA<ConfigException>()));
    expect(() => Config.load({'CAPTIONS_CONFIG': writeConfig('[1,2]')}), throwsA(isA<ConfigException>()));
    expect(
      () => Config.load({'CAPTIONS_CONFIG': writeConfig('{"INGEST_TOKEN": {"nested": true}}')}),
      throwsA(isA<ConfigException>()),
    );
  });

  test('on Cloud Run (K_SERVICE) the dev-default ingest token is refused', () {
    expect(() => Config.load({'K_SERVICE': 'captions'}), throwsA(isA<ConfigException>()));
    expect(() => Config.load({'K_SERVICE': 'captions', 'INGEST_TOKEN': 'dev-token'}), throwsA(isA<ConfigException>()));
    final config = Config.load({'K_SERVICE': 'captions', 'INGEST_TOKEN': 'real-secret'});
    expect(config.ingestToken, 'real-secret');
  });

  test('describe() never contains secret values', () {
    final config = Config.load({'GEMINI_API_KEY': 'super-secret', 'INGEST_TOKEN': 'another-secret'});
