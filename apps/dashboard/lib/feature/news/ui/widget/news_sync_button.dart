...
import 'package:dashboard/feature/news/data/provider/news_sync_service.dart';
import 'package:flutter/material.dart';
import 'package:flutter_hooks/flutter_hooks.dart';
import 'package:go_router/go_router.dart';
import 'package:hooks_riverpod/hooks_riverpod.dart';

/// News 全データを本番環境へワンクリックで反映するボタン。
///
/// 実行前に dry run で反映内容（作成・更新・削除の件数）を取得して確認ダイアログを
/// 表示し、承認後に Cloud Functions (`syncNewsToProd`) を実行する。
/// 本番フレーバーでは反映元にならないため表示しない。
class NewsSyncButton extends HookConsumerWidget {
  const NewsSyncButton({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Future<void> sync() async {
      final service = ref.read(newsSyncServiceProvider);
      isSyncing.value = true;
      try {
        // まず dry run で反映内容を取得し、確認ダイアログを表示する。
        final plan = await service.syncToProd(dryRun: true);
        if (!context.mounted) return;
        final confirmed = await _NewsSyncConfirmDialog.show(context, plan: plan);
...
class _NewsSyncConfirmDialog extends StatelessWidget {
  const _NewsSyncConfirmDialog({required this.plan});

  final NewsSyncResult plan;

  static Future<bool?> show(BuildContext context, {required NewsSyncResult plan}) {
    return showDialog<bool>(
      context: context,
      builder: (_) => _NewsSyncConfirmDialog(plan: plan),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      content: Column(
        ...
        children: [
          Text('ニュース全データ（全 ${plan.total} 件）を本番環境へ反映します。'),
